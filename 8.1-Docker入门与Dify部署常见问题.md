...
这篇文档不是一章“单独的大课”，而更像这一组部署文档里的**基础设施说明书 + 排障手册**。它最适合和下面两章一起配合着看：

- [第 7 章 Dify 的 Windows 平台部署](7-Dify的Windows平台部署.md)：告诉你怎么把 Dify 跑起来
- [第 8 章 企业级大模型部署](8-企业级大模型部署.md)：告诉你企业环境里怎么把应用层和模型层拆开部署

---

**本章课程目标：**

- 理解 Docker 实操中最容易混淆的几个基础概念：**镜像、容器、数据卷、Compose**。
- 结合 Dify 部署场景，理解容器和宿主机目录的关系，知道数据通常存在哪里、为什么删除容器不一定丢数据。
- 掌握一组最常用的 Docker / Docker Compose 排查命令，并能把这篇文档当作部署与运维时的快速参考手册。

**学习建议：** 不必从头到尾死记命令。更推荐的读法是：先看「阅读前先搞清楚三件事」和第 1~4 节，把镜像、容器、数据卷、Compose 的关系搞清楚；真正部署时，再按问题回跳到备份、升级、数据库连接、前端改动等对应小节。
...
![Docker 与 Dify 部署操作截图](images/8.1/8.1-1-1.png)

### 1.1 如何区分镜像和容器？

在 Docker Desktop 里可以从两个界面直观看到「容器」和「镜像」的分别：

- **容器列表**：Docker Desktop 的 **Containers（容器）** 界面。每一行是一个**运行中的容器**，能看到容器名（Name）、所基于的镜像（Image）、端口映射、CPU 占用等；这里的 Name 列就对应命令行 `docker ps` 的 NAMES 列。
- **镜像列表**：Docker Desktop 的 **Images（镜像）** 界面。这里列出的是本地的**镜像**（即“安装包/模板”），有镜像名、标签（Tag）、大小等；镜像本身不运行，只有用镜像创建并启动后，才会在「容器」列表里出现。

![Docker 与 Dify 部署操作截图](images/8.1/8.1-1-2.png)

_图：Docker Desktop — Containers（容器）列表，每一行对应一个运行中的容器。_

![Docker 与 Dify 部署操作截图](images/8.1/8.1-1-3.png)

_图：Docker Desktop — Images（镜像）列表，此处为本地镜像，是创建容器的模板。_
...
这里可以把镜像类比成 Windows 安装 ISO、Node 的 npm 包，或者手机 App 的安装包 APK。

它本身不会运行，只负责提供一套“运行所需的文件、依赖和默认启动命令”。

> **✅ 容器（Container）= “运行起来的实例 / 进程”**

如果沿用刚才的类比，镜像相当于“装好了微信”，容器则是“把微信真正打开并运行起来”。

容器本质上是一个正在运行的实例，所以它会表现为进程，带有自己的日志、端口和内存占用，也可以被启动、停止、重启或删除。

**核心关系一句话：** 镜像是模板，容器是从模板创建并运行的实例，而且一个镜像可以启动多个容器。
...
**它的作用很明确：**用 `docker-compose.yaml` 描述要启动哪些服务、每个服务使用什么镜像、容器之间如何组网、端口怎样映射，以及数据最终挂载到哪里。
...
这里的 `langgenius/dify-web:1.11.4`，就像一个已经执行过 `npm install`、`npm build` 的完整产物。镜像内部可能长这样：
...
docker-compose 只是说：运行这个镜像、给它传环境变量、给它挂载数据目录、给它配置网络。**它没有读取本地仓库里的 web 源码。**
...
![Docker 与 Dify 部署操作截图](images/8.1/8.1-5-4.png)
...
![Docker 与 Dify 部署操作截图](images/8.1/8.1-5-3.png)
...
![Docker 与 Dify 部署操作截图](images/8.1/8.1-5-2.png)
...
![Docker 与 Dify 部署操作截图](images/8.1/8.1-5-1.png)
...
- <strong>-c "\l"</strong>：执行一条命令；`\l` 为 PostgreSQL 内部命令，表示 <strong>列出所有数据库</strong>。  
...
- **dockerfile** 路径以当前仓库实际结构为准，常见为 `web/Dockerfile`；若不同，改为实际路径即可。

### 8.4 第 3 步：修改前端源码

前端源码在仓库根目录下的 <strong>web/</strong>（一般为 React/Next 等），例如：
...
1. 镜像、容器、数据卷三者在 Docker 体系里的关系是什么？

   **答案：** 镜像是应用模板，容器是镜像运行后的实例，数据卷是容器外部的持久化存储。最直观的类比是：镜像像安装包，容器像正在运行的程序，数据卷像独立保存数据的硬盘目录。

2. 为什么很多 Dify 部署命令都要求你先进入项目的 docker 目录？

   **答案：** 因为 `docker-compose.yaml`、`.env` 等部署文件通常就在项目的 `docker` 目录里，很多命令都依赖当前目录下的这些配置文件来确定要启动哪些服务。目录进错了，命令就可能找不到配置或启动错东西。

3. 如果担心删容器后丢数据，第一件事应该确认什么？

   **答案：** 先确认数据到底是放在容器内部，还是放在数据卷或挂载目录里。只要真正的数据保存在卷里，删容器通常不会把数据一起删掉。

4. 如果让你用最少术语解释“为什么删容器不一定丢数据”，你会怎么讲？

   **答案：** 因为容器只是“运行中的壳”，真正长期要保留的数据通常会单独放在数据卷里。删掉壳不一定会删掉外面的存储，所以前提是你得确认数据确实没有写在容器内部。

5. 以后再遇到 Dify 部署问题时，你会如何判断是 Docker 基础概念没理清，还是具体服务本身出了问题？

   **答案：** 我会先判断问题是不是出在镜像、容器、数据卷这些基本概念没搞清，再看 Dify 的具体服务配置、日志和依赖有没有报错。也就是先分清“Docker 层”还是“应用层”，这样排查才不会混在一起。

**本章小结：**

- **概念层**：镜像像安装包，容器像跑起来的进程，数据卷负责把关键数据和容器生命周期拆开。
- **实操层**：Dify 的很多排障问题，本质上不是 Dify 特有问题，而是对 Docker、Compose、挂载和日志不够熟。
- **使用方式**：这份文档最适合当“排错手册”和“概念补丁”，遇到问题按主题快速回跳，而不是一次性死记所有命令。
- 从掌握结果看，学完本章后，你至少应该：能分清镜像、容器、数据卷、Compose 这几个最基础但最容易混淆的 Docker 概念；能理解 Dify 部署场景里“数据到底存在哪、删容器为什么不一定删数据”这类高频问题；能使用一组常见 Docker 命令做基础排查、查看日志、查看挂载和定位容器。

**建议下一步：** 如果当前正在部署 Dify，本章最适合和 [第 7 章 Dify 的 Windows 平台部署](7-Dify的Windows平台部署.md) 或 [第 8 章 企业级大模型部署](8-企业级大模型部署.md) 对照着看；如果已经把部署跑通，也可以把这里的命令清单当成之后排障时的长期参考。
