...
- 题目下方尽量补充了**对应章节**，方便结合本套教程回看知识点、边学边练。
...
**对应章节：**[1-3 RAG、微调、续训与智能体 §1、RAG](1-3-RAG、微调、续训与智能体.md#_1、rag)；[1-3 RAG、微调、续训与智能体 §4、智能体开发](1-3-RAG、微调、续训与智能体.md#_4、智能体开发)

### Q1-2. RAG 和微调（SFT）的核心区别是什么？能不能一起用？
...
**对应章节：**[1-3 RAG、微调、续训与智能体 §1、RAG](1-3-RAG、微调、续训与智能体.md#_1、rag)；[1-3 RAG、微调、续训与智能体 §2、微调（Fine-tuning）](1-3-RAG、微调、续训与智能体.md#_2、微调%EF%BC%88fine-tuning%EF%BC%89)
...
### Q1-4. 什么是 AI Agent？它和传统 AI 应用的区别是什么？

更稳妥的概括方式是：**Agent = 模型 + 工具集 + 运行循环 + 当前状态**。
...
**对应章节：**[21 Agent智能体 §1、Agent 简介](21-Agent智能体.md#_1、agent-简介)；[1-3 RAG、微调、续训与智能体 §4、智能体开发](1-3-RAG、微调、续训与智能体.md#_4、智能体开发)
...
**对应章节：**[1-2 提示词工程 §4、提示词工程的边界](1-2-提示词工程.md#_4、提示词工程的边界)；[13 提示词与消息模板 §1、Prompt 简介](13-提示词与消息模板.md#_1、prompt-简介)
...
**对应章节：**[13 提示词与消息模板 §8、从文件加载提示词](13-提示词与消息模板.md#_8、从文件加载提示词)；[1-2 提示词工程 §5、提示词工程的几个注意点](1-2-提示词工程.md#_5、提示词工程的几个注意点)

### Q2-6. Prompt Engineering 中，如何系统优化提示词并把准确率做上去？
...
**对应章节：**[1-2 提示词工程 §2、提示词怎么写](1-2-提示词工程.md#_2、提示词怎么写)；[14 输出解析器 §8、实际开发选择](14-输出解析器.md#_8、实际开发选择)
...
通常不会先看榜单，而是先看自己的知识类型和检索目标，再反推模型选型。更稳妥的思路是先看三件事：
...
真正评估时，我更关注“它有没有把你的检索链路整体抬起来”，而不是只看一个榜单分数。比较实用的判断通常有三层：
...
### Q4-3. 什么是向量数据库？它和传统数据库有什么区别？为什么不能只用 MySQL？

如果先下一个定义，向量数据库就是一种专门面向“相似度检索”的存储系统，用来保存向量、原始内容和元数据，并支持近邻搜索、过滤和召回。
...
这里还需要补充一个重要点：现代 Tool Calling Agent 不一定会把 `Thought` 明文展示出来。实际代码中更常见的是 `AIMessage.tool_calls`、工具执行结果和下一轮消息。因此，更稳妥的理解方式是：**ReAct 更接近一种工作机制，而不只是某个固定 Prompt 模板。**
...
这类问题不宜一开始就引入复杂 Agent，而应先判断它是否属于典型的工具型助手场景。像查天气、查新闻这种需求，核心不是复杂规划，而是“意图识别 + 工具调用 + 结果整合”。
...
评估可以先拆成**离线评测**和**在线评测**，再把问题拆到具体层级，而不是只盯一个总分。
...
所以我更认可的评测闭环是：**先分层建指标，再沉淀 badcase，最后让 badcase 反过来驱动 Prompt、检索、工具和流程迭代。**
...
如果按这套仓库的主线去理解，它本质上更接近“**工作流 / 图编排 + 多轮检索 + 报告生成**”的组合能力，而不是把普通 RAG 的上下文塞得更长。重点不只是召回，而是有没有把研究过程拆开、证据有没有被反复求证、最后能不能形成带引用的结构化结论。
...
换句话说，纯软件工具型 Agent 更接近“调接口、读写数据、操作页面”，重点是权限、状态机和工具编排；真实环境 Agent 则多了感知、定位、动作执行和安全约束，很多时候不是“调用成功”就算完成，而是还要判断动作有没有真正生效、环境有没有变化、风险是否可控。
...
- 仓库理解层：通过代码搜索、文件树、符号索引、依赖关系、提交历史建立最小必要上下文，而不是把整个仓库一股脑塞进模型。
...
- 上下文治理：仓库很大时，怎么只拿到和当前任务最相关的上下文。
- 验证闭环：怎么判断修复真的生效，而不是只让模型说“我已经改好了”。
- 失败处理：当 Agent 连续修不好 bug 时，怎么停下来、怎么暴露中间诊断、怎么交还给人。

所以一个成熟的 Code Agent，更接近“面向代码任务的执行系统”，而不是“代码自动补全工具放大版”。它的关键不只是会写代码，而是能把 **仓库理解、工具调用、验证反馈、停止条件** 做成稳定闭环。
...
**考察点：**代码场景下的 Agent 设计能力、仓库上下文治理能力，以及“修改 - 验证 - 继续修复”的闭环意识。
...
因此，不宜简单判断谁更强，而应回到任务类型本身。如果是“边写边补全、快速改一段代码”，IDE 内工具体验更顺；如果是“要理解整个仓库、跑命令、跨文件修改、完成一个更完整任务”，终端型 Agent 更有优势；如果团队已经有稳定 IDE 习惯，轻量补全类工具的接入成本也更低。
