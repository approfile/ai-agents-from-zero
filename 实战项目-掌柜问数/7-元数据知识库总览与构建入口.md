...
前面几章，我们已经把「掌柜问数」运行所需的基础设施准备好了，包括：`MySQL`、`Qdrant`、`Elasticsearch`、`Embedding`、配置管理、日志管理、客户端封装。

这些内容本质上都在做一件事：**为真正的业务逻辑做准备。**前面第 1 章和第 2 章，已经分别讲清楚了「掌柜问数」要解决什么问题，以及这套系统整体是怎么运转的。因此从这一章开始，我们不再重复项目背景，而是把视角正式切进第一条核心业务线：**构建元数据知识库**。

你也可以把这一章先理解成“元数据知识库篇章的总入口”。它要解决的，不是某一个具体入库细节，而是先把下面这几个前置问题讲清楚：
...
![掌柜问数系统整体架构：数据仓库、配置与同步脚本、元数据知识库与问数智能体](images/2/2-1-2-1.png)
...
所以项目里采用了一种非常经典的分层思路。如果你做过常规后端开发，可以先类比成下面这个结构：
...
这些问题问得非常好，因为后面第 8 章真正展开代码时，主线其实就是在这 4 类角色之间流转。
...
它的作用是把来自不同地方的信息，统一组织成系统后续都能复用的结构。例如：

- 一部分信息来自配置文件
- 一部分信息来自数仓查询
- 后面还可能继续被拿去写 MySQL、写 Qdrant、写 Elasticsearch

所以业务实体关注的重点不是“怎么存数据库”，而是：**系统内部如何统一表达一个表、一个字段、一个指标。**

如果你做过前端开发，可以把它类比成一个运行时真实存在的 DTO / domain object。它有点像 TS 里的类型定义，但它不只是静态类型提示，而是程序运行时真的会创建出来的 Python 对象。
...
换句话说：业务实体关心“系统内部如何表达”，ORM 模型关心“数据库表结构如何映射”。这两个对象看起来字段可能很像，但职责完全不一样。
...
所以，“元数据知识库”这个词，在「掌柜问数」里并不是指某一个数据库，而是指一整套为问数服务的知识组织方案。

先从同步脚本入口本身看起，项目中的同步脚本路径是：`shopkeeper-agent-backend/app/scripts/build_meta_knowledge.py`
...
也就是说，“同步脚本的作用”更准确地说，是**把整条元数据知识库构建链路调度起来**，而不是把所有构建细节都直接写在脚本里。
真正的元数据知识库构建主流程，位于：`shopkeeper-agent-backend/app/services/meta_knowledge_service.py`

这是一种很典型、也很值得初学者养成的工程习惯：**入口只负责调度，复杂业务逻辑沉淀到服务层。**

如果你想再往代码上靠近一步，可以先看一下 `MetaKnowledgeService.build(...)` 的编排骨架。把细节先省略掉，它的主流程大致就是这样：

项目对应文件路径：`shopkeeper-agent-backend/app/services/meta_knowledge_service.py`
...
这段代码非常值得先建立整体印象，因为后面第 8 章到第 11 章，本质上就是在逐段展开这几行流程。
...
脚本负责“怎么做”、配置负责“做哪些内容”。放到「掌柜问数」里，具体就是由配置文件来决定：
...
Traceback (most recent call last):
  File ".../app/clients/qdrant_client_manager.py", line 7, in <module>
    from app.conf.app_config import QdrantConfig, app_config
ModuleNotFoundError: No module named 'app'
```

同步脚本 `app/scripts/build_meta_knowledge.py` 也是同样的道理。只要文件里用了这种绝对导入：
...
理解这个问题，抓住 `sys.path` 就够了。你可以把它理解成：**Python 启动后，用来查找模块的一组路径清单。**
...
解释器更容易把脚本所在目录 `app/scripts/` 放进模块搜索路径。但我们真正想让它识别的，是项目根目录 `shopkeeper-agent-backend/`，因为 `app` 包是放在这里下面的。
...
- 解释器从 `app/scripts/` 开始找模块
- 但这个目录下面并没有一个上层包叫 `app`
- 于是 `from app...` 这样的绝对导入就失败了
...
也就是说，`sys.argv[1]`、`sys.argv[2]` 本质上就是“按位置硬取命令行参数”的写法。它在极简单的脚本里能用，但只要参数顺序变化、参数变多，或者需要支持 `-c` / `--conf` 这种可选参数时，就会越来越不稳。
...
- 语义更清楚，别人一看就知道这是路径，不是普通文本
- 后续做拼接、判断是否存在、读取文件时更方便
...
它是给人看的配置文件。你可以把它理解成一份“同步清单”，里面声明：
...
- 服务层可以专注于“先做什么、后做什么”
- 存储读写逻辑不会散落在各个业务函数里
- 后面如果 SQL、ORM、索引写入方式发生变化，调整范围会更集中
...
项目对应文件路径：`shopkeeper-agent-backend/conf/meta_config.yaml`
...
项目对应文件路径：`shopkeeper-agent-backend/app/scripts/build_meta_knowledge.py`
...
### 9.2 为什么服务层要持有 repository，而不是直接持有 session
...
## 10、MetaConfig 和 OmegaConf 是如何把配置读进系统的
...
### 10.1 MetaConfig 到底起什么作用

项目对应文件路径：`shopkeeper-agent-backend/app/conf/meta_config.py`
...
你可以把它理解成一句话：**`MetaConfig` 在定义“这份配置文件在程序里应该长成什么样”。**
...
#### 10.2.1 OmegaConf.load(config_path)：先把 YAML 文件读出来
...
#### 10.2.2 OmegaConf.structured(MetaConfig)：先准备好结构模板

```python
schema = OmegaConf.structured(MetaConfig)
```

这一步可以理解成：“先告诉程序，合法的配置应该符合 `MetaConfig` 这套结构。”也就是说，这一步是在准备一份“标准答案的结构模板”。

#### 10.2.3 merge + to_object：把内容套进结构里
