...
- 理解「掌柜问数」项目为什么使用 `uv` 管理 Python 环境与依赖。
...
在正式动手之前，建议先知道「掌柜问数」这套工程大致由哪些部分组成。

当前工程里最重要的两个项目是：

- 后端项目：`shopkeeper-agent-backend（又名：data-agent）`
- 前端项目：`shopkeeper-agent-frontend（又名：data-agent-fronted）`

不过这一章的重点会放在**后端环境和基础服务准备**上，**不展开讲前端项目的创建过程**。前端代码在实际学习时，可以直接从对应的 Git 仓库拉取下来使用。

其中：

- 后端使用 `Python 3.12+`，并通过 `uv` 管理虚拟环境和依赖。
- 前端使用 `Vue 3 + Vite`，需要 `Node.js` 和 `npm`。
...
一句话概括：**在「掌柜问数」中，`uv` 既负责管理虚拟环境，也负责管理项目依赖，是后端开发环境的基础工具。**
...
```bash
didilili@DidililiMacBook-Pro shopkeeper-agent-backend % uv --version
...
这一节主要讲后端项目 `shopkeeper-agent-backend` 的创建方式。
...
1. 选择 `New Project`
2. 在解释器类型中选择 `uv`
3. 指定项目目录，例如 `shopkeeper-agent-backend`
4. 选择 Python 版本，建议与当前项目保持一致，使用 `3.12`
5. 点击 `Create`
...
```bash
uv init shopkeeper-agent-backend
```
...
```bash
# Windows
uv add fastapi[standard] sqlalchemy asyncmy qdrant-client "elasticsearch[async]>=8,<9" langchain langchain-deepseek langchain-huggingface langgraph jieba omegaconf pyyaml loguru cryptography huggingface-hub
# macOS
uv add "fastapi[standard]" sqlalchemy asyncmy qdrant-client "elasticsearch[async]>=8,<9" langchain langchain-deepseek langchain-huggingface langgraph jieba omegaconf pyyaml loguru cryptography huggingface-hub
...
> [8.1 - Docker入门与Dify部署常见问题](../8.1-Docker入门与Dify部署常见问题.md)
...
常见原因是默认镜像源访问不稳定（由于众所周知的原因），因此一般有两种解决思路：
...
![Docker Desktop 的 Docker Engine 配置界面与 registry-mirrors 示例](images/3/3-4-5-1.png)
...
在「掌柜问数」里，这份 `Compose` 文件可以理解成：一份基础服务清单。也就是说，它描述的不是某一个容器，而是“让这个项目跑起来，需要哪几类容器协同工作”。
