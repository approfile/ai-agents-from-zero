# 4 - 掌柜问数：项目结构与基础服务配置管理

---

**本章课程目标：**

- 先建立对 `shopkeeper-agent-backend` 项目结构的整体认识，知道代码、配置、提示词和脚本分别放在哪里。
...
这一章开始，我们正式进入「掌柜问数」的**基础服务搭建**部分。

这里的“基础服务”可以先理解为三类和业务无关、但项目运行离不开的内容：
...
当前仓库中，后端项目 `shopkeeper-agent-backend` 的核心结构如下：
...
如果你以前做过 Java 或 Spring Boot 开发，也可以先这样类比：
...
用于读取配置文件的代码放置在 `shopkeeper-agent-backend/app/conf/app_config.py` 文件中，你也可以根据以下路径在对应文件夹下创建对应文件。
...
```bash
(shopkeeper-agent-backend) didilili@DidililiMacBook-Pro shopkeeper-agent-backend % python3 app/conf/app_config.py
localhost
```
...
- `OmegaConf.structured(AppConfig)` + `OmegaConf.load(...)` + `OmegaConf.merge(...)`
  这一组操作，本质上就是在做“先定义结构，再加载内容，最后把它们合并成一个配置对象”。
...
- **绝对路径**
  是从系统根目录开始写出的完整路径，例如：
  `/Users/tools/Desktop/agent/shopkeeper-agent-backend/conf/app_config.yaml`
- **相对路径**
...
如果你恰好是在 `shopkeeper-agent-backend/` 根目录启动程序，这个路径可能是对的。  
...
```python
/Users/tools/Desktop/agent/shopkeeper-agent-backend/app/conf/app_config.py
```
...
- 当前文件是 `shopkeeper-agent-backend/app/conf/app_config.py`
- 上一级目录是 `shopkeeper-agent-backend/app/conf`
- 上两级目录是 `shopkeeper-agent-backend/app`
- 上三级目录是 `shopkeeper-agent-backend`
...
定位到的就是 `shopkeeper-agent-backend` 项目根目录。接下来这一段：
...
```text
shopkeeper-agent-backend/conf/app_config.yaml
```

所以整行代码的完整含义就是：先从当前 `app_config.py` 文件出发，稳定地回到 `shopkeeper-agent-backend` 根目录，再进入 `conf/app_config.yaml`，从而拿到配置文件的绝对路径。
...
当前文件 app/conf/app_config.py
  -> 回到项目根目录 shopkeeper-agent-backend
  -> 进入 conf 目录
  -> 找到 app_config.yaml
```
...
- 从项目结构上看，后端主项目 `shopkeeper-agent-backend` 采用了比较清晰的分层组织：`clients` 负责外部依赖、`repositories` 负责数据访问、`services` 负责业务逻辑、`api` 负责接口暴露。
...
下一篇会正式进入检索基础服务，也就是 **Qdrant 与 Elasticsearch**。前者负责**语义相似度召回**，后者负责**全文文本检索**，它们一起构成了「掌柜问数」在检索层的核心能力。
