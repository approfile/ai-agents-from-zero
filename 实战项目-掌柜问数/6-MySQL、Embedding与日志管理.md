# 6 - 掌柜问数：MySQL、Embedding 接入与日志管理
...
在「掌柜问数」里，我们并不是直接把自然语言问题交给大模型就结束了。在字段召回、指标召回、元知识构建这些环节里，系统都需要先把文本转成向量，再交给 `Qdrant` 做相似度检索。也就是说，这里有一条很重要的链路：**文本 -> Embedding 服务 -> 向量 -> Qdrant 检索**。如果没有这一层，后面的向量召回链路就跑不起来。
