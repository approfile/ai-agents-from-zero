...
**学习建议：** 这篇不要贪多，先选一条路线跑通。只想体验 Coze，就先完成 Docker Desktop、Coze Studio 和模型配置；只想体验 Dify，就先把 Dify 服务启动并能登录。Coze Loop 可以等 Studio 跑通后再看。模型配置是最容易卡住的地方：选火山方舟就提前准备 API Key 和 Endpoint，选 Ollama 就先确认本地模型已经拉取成功。
